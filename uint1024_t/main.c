...
//    }
  //  return multiplification;
    struct uint1024_t mult = from_uint(0);
    for(int i = 34; i > -1; --i){
         int additive = 0;
        for ( int j = 34; i != -1; i--){
            if (i + j > 35) break;
            unsigned long t =mult.array[i + j] + x.array[i] * y.array[i] + additive;
            mult.array[i + j]= t % DIV;
            additive = t / DIV;
        }
    }
    return mult;
...
void scanf_value(struct uint1024_t *x) {
    ...
    //while (1) {
    scanf("%s", &tempArray);
    //  if (tempArray[0] == '0' || tempArray[0] == '-') printf("THE ENTERED VALUE IS INVALID\n"); else break;
    //}
